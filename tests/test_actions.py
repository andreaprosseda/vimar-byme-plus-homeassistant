...
Il banco (`entry_caricata`, `client`, `eid`) e' condiviso con test_setup.py —
vive in conftest.py e in helpers/ha_harness.py. Due regole per non legare i
test a dettagli che cambiano tra le versioni:

- gli entity_id NON si scrivono a mano: si risolvono con la fixture `eid`
  (dominio + caso d'uso -> entity_id vero, via unique_id). Vedi il perche'
  nel docstring di `eid` in conftest.py: da HA 2026.9 il core antepone il
  nome del dispositivo all'entity_id.
- `ActionType` si importa con `helpers.ha_harness.action_type_prefissato()`,
  non con uno statico in testa al file: stesso motivo dei due universi di
  moduli risolto per VimarDataMapper.
"""
...
def _oggetto_entita(hass: HomeAssistant, entity_id: str):
...
async def test_luce_turn_on(hass, entry_caricata, client, eid):
    ActionType = action_type_prefissato()
    await _comanda(hass, "light", "turn_on", eid("luce_spenta", "light"))
    _id, tipo, _args = _ultima(client)
    assert tipo is ActionType.ON


async def test_luce_turn_off(hass, entry_caricata, client, eid):
    ActionType = action_type_prefissato()
    await _comanda(hass, "light", "turn_off", eid("luce_accesa", "light"))
    _id, tipo, _args = _ultima(client)
...
async def test_switch_turn_on(hass, entry_caricata, client, eid):
    ActionType = action_type_prefissato()
    await _comanda(hass, "switch", "turn_on", eid("automazione_spenta", "switch"))
    _id, tipo, _args = _ultima(client)
    assert tipo is ActionType.ON
...
async def test_cover_open(hass, entry_caricata, client, eid):
    ActionType = action_type_prefissato()
    await _comanda(hass, "cover", "open_cover", eid("cover_chiusa", "cover"))
    _id, tipo, _args = _ultima(client)
    assert tipo is ActionType.OPEN


async def test_cover_close(hass, entry_caricata, client, eid):
    ActionType = action_type_prefissato()
    await _comanda(hass, "cover", "close_cover", eid("cover_aperta", "cover"))
    _id, tipo, _args = _ultima(client)
...
async def test_cover_set_position_inverte_la_scala(
    hass, entry_caricata, client, eid, posizione_ha, posizione_vimar_attesa
):
    ...
    await _comanda(
        ...
        "cover",
        "set_cover_position",
        eid("cover_aperta_50_percento", "cover"),
...
async def test_clima_set_temperature(hass, entry_caricata, client, eid):
...
async def test_clima_set_hvac_mode_con_permessi(hass, entry_caricata, client, eid):
    ActionType = action_type_prefissato()
    await _comanda(
        hass,
        "climate",
...
CASO_SENZA_PERMESSI = "clima_senza_permessi_cambio_modo"


async def test_clima_senza_permessi_si_puo_spegnere(hass, entry_caricata, client, eid):
    """Spegnere non tocca il change-over: il grant non c'entra."""
    ActionType = action_type_prefissato()
    await _comanda(hass, "climate", "turn_off", eid(CASO_SENZA_PERMESSI, "climate"))
    _id, _tipo, args = _ultima(client, ActionType.SET_HVAC_MODE)
    assert args == ("off",)


async def test_clima_senza_permessi_si_puo_riaccendere(
    hass, entry_caricata, client, eid
):
    """Riaccendere nella stagione gia' impostata dall'impianto neppure."""
    ActionType = action_type_prefissato()
    await _comanda(hass, "climate", "turn_on", eid(CASO_SENZA_PERMESSI, "climate"))
    _id, _tipo, args = _ultima(client, ActionType.SET_HVAC_MODE)
...
async def test_clima_senza_permessi_rifiuta_il_cambio_stagione(
    ...
    """
    entita = _oggetto_entita(hass, eid(CASO_SENZA_PERMESSI, "climate"))
...
async def test_clima_set_fan_mode(hass, entry_caricata, client, eid):
    ActionType = action_type_prefissato()
    await _comanda(
        hass, "climate", "set_fan_mode", eid("clima_caldo", "climate"), fan_mode="high"
    )
...
async def test_button_press(hass, entry_caricata, client, eid):
    ActionType = action_type_prefissato()
    await _comanda(hass, "button", "press", eid("scena_a_riposo", "button"))
