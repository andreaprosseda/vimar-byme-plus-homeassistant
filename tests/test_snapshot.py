...
def snapshot(snapshot: SnapshotAssertion) -> SnapshotAssertion:
    """Lo snapshot con il serializer di Home Assistant.

    Senza, i campi volatili di RegistryEntry/State (`created_at`, `id`,
    `config_entry_id`, `device_id`, i timestamp...) finiscono nel file come
    valori letterali invece che come `<ANY>`, e lo snapshot fallisce alla
    generazione successiva perche' quei valori cambiano ad ogni run. Fino al
    plugin 0.13.316 questa estensione era applicata di default; dalle versioni
    piu' recenti va richiesta esplicitamente — questo la mette su entrambe.
    """
