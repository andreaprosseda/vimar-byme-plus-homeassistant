...
all'ultima versione, con la `homeassistant` 2026.9.x che porta con se') lo
richiede. Verificata su Python 3.14.2 + homeassistant 2026.9.1 + plugin 0.13.364.
...
di `requirements-test.txt`, rigenera `tests/fixtures/master.db` e fa girare
pytest. Le volte dopo riusa lo stesso `.venv/`.

Se il tuo `python3` non e' gia' 3.14, indica l'interprete con `PYTHON`:

```bash
PYTHON=python3.14 tests/run.sh
PYTHON="$(uv python find 3.14)" tests/run.sh   # se usi uv per gestire i Python
```
...
Il CONTENUTO e' deterministico (stesso generatore → stesse righe), ma i BYTE
del file no: dipendono dalla versione di SQLite bundlata con Python, quindi lo
stesso generatore su Python diversi produce md5 diversi. Per questo ne' la CI
ne' `run.sh` confrontano i byte: rigenerano il file e basta. La deriva che
conta — qualcuno cambia i casi d'uso ma non rigenera gli snapshot — la coglie
`test_snapshot.py`, che confronta il contenuto delle entita', non il database.

Il `master.db` committato serve solo a chi lancia `pytest` a mano senza
rigenerarlo; e' stato generato su Python 3.14.
