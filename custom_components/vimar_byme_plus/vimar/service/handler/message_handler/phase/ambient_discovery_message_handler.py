...
class AmbientDiscoveryMessageHandler(BaseMessageHandler):
    ...
    def handle_message(self, message: BaseRequestResponse, config: MessageSupportingValues) -> BaseRequestResponse:
        log_info(__name__, "Ambient Discovery Phase completed, sending SF Discovery Request...",)
        self.save_ambients(message)
        return self.get_sf_discovery_request(config)

    def get_sf_discovery_request(self, config: MessageSupportingValues) -> SfDiscoveryRequest:
